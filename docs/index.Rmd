...
read.table('/project/sequencing_qc.txt', sep = '\t', header = T) %>% # this table is stored locally
  ...
  geom_point() +   
  ggrepel::geom_label_repel(aes(label = Sample), nudge_x = 0.3, direction = 'y') +
  theme_bw() +
  labs(x = '', y = '') +
  scale_fill_manual(values = c('Number of cells' = 'black',
                               'Yield (Mbases)' = 'cadetblue', 
                               '% >= Q30 bases' = 'gold',
                               'Mean Quality Score' = 'salmon'),
                    guide = 'none') +
  theme(axis.ticks.x = element_blank(),
        axis.text.x = element_blank()) +
  facet_wrap(vars(metric), scales = 'free', nrow = 1)

ggsave('/project/analysis/plots/scrnaseq_qc.png', width = 25, height = 15, units = 'cm', dpi = 300)
...
```{r, echo = T, eval = F}
# emap plot
emapplot(go_male_vs_untr, color = 'qvalue', pie.params = list(pie = 'count'), cex.params = list(category_node = 1)) + 
